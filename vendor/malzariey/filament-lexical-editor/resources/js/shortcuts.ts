...
export function isStartAlign(event: KeyboardEvent): boolean {
    const {code, shiftKey, altKey, metaKey, ctrlKey} = event;
    return (
        code === 'BracketLeft' && shiftKey && !altKey && controlOrMeta(metaKey, ctrlKey)
    );
}

export function isEndAlign(event: KeyboardEvent): boolean {
    const {code, shiftKey, altKey, metaKey, ctrlKey} = event;
    return (
        code === 'BracketRight' && shiftKey && !altKey && controlOrMeta(metaKey, ctrlKey)
    );
}
